...
export default function Floors() {
  ...
  const openCreate = () => { setEdit(null); setForm({ name: '' }); setModal(true); };
  const openEdit = (floor) => { setEdit(floor); setForm({ name: floor.name }); setModal(true); };

  const handleSave = async () => {
    try {
      if (edit) {
        await updateFloor(edit.id, form);
  ...
  return (
    <div className="max-w-[1280px]">
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-display text-text-primary">Floors</h1>
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="relative flex-1 max-w-sm">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search floors..." className="w-full border border-border rounded-sm pl-9 pr-8 py-2 text-body bg-bg-app focus:outline-none focus:border-accent focus:ring-[3px] focus:ring-accent-soft" />
      ...
      <div className="bg-bg-app border border-border rounded-md shadow-sm">
        <DataTable columns={columns} data={data} loading={loading} emptyMessage="No floors match your search" />
      </div>
      {modal && (
        <Modal title={edit ? 'Edit floor' : 'New floor'} onClose={() => setModal(false)}>
